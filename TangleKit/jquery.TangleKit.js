...
// Extend Number with the .round function
...
Tangle.classes.TKToggle = {
    ...
    initialize: function (element, options, tangle, variable) {
        ...
        $(element).
            click(toggle).
            keypress(function(e) {
                // Flip on space or enter keypress
                // Add `tabindex="0"` to the element in your HTML so that the
                //   element can be focused using the Tab key
...
Tangle.classes.TKNumberField = {
    ...
    initialize: function (element, options, tangle, variable) {
        var ref = this;
        this.sizing = $(document.createElement("span")).
            addClass("TKNumberFieldInputSizing").
        ...
        this.input = $(document.createElement("input")).
            ...
            css("width", (options.size || 6) + "em").
            addClass("TKNumberFieldInput").
            insertBefore(element).
            bind("keyup blur change input", function() {
                var value = ref.getValue();
                ref.sizing.text(value);
                ref.input.width(ref.sizing.width() + 4);
    ...
    update: function (element, value) {
        ...
        if (value !== currentValue) {
            this.sizing.text(value);
            this.input.width(this.sizing.width() + 4);
...
//  TKSelect
//
//  Handle a SELECT element through Tangle, getting and setting values based
//  on the OPTION tags.
//
...
Tangle.classes.TKSelect = {
    initialize: function(element, options, tangle, variable) {
        var ref = this;
        this.input = $(element);
        this.input.bind("keyup blur change", function() {
            tangle.setValue(variable, ref.getValue());
        });
    },
...
//----------------------------------------------------------
//
//  TODO: This portion has not been modified to work with jQuery yet
//
//  TKAdjustableNumber
//
...
Tangle.formats.dollars = function (value) {
    return "$" + value.round(2).replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1,");
};
